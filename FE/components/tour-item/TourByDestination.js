    ...
    >
      ...
      >
        <div className="image">
          ...
          <a href="#" className="heart" style={{ color: '#ff4d4d' }}> {/* Red heart icon */}
        ...
        <div className="content" style={{ padding: '20px' }}> {/* Add some padding inside the content area */}
          <div className="destination-header">
            <span className="location" style={{ color: '#555' }}> {/* Darker grey for location text */}
              <i className="fal fa-map-marker-alt" style={{ color: '#007bff' }} /> {/* Blue map marker icon */}
              {" "}
              {location}
            </span>
            <div className="ratting">
              {Array.from({ length: Math.round(rating || 0) }).map(
                (_, index) => (
                  <i key={index} className="fas fa-star" style={{ color: '#ffc107' }} /> // Gold stars
                )
              )}
              {rating > 0 && (
                <span className="ms-1" style={{ color: '#777' }}>({rating.toFixed(1)})</span> // Lighter grey for rating number
              )}
            </div>
          </div>
          <h5>
            <Link href={`/tour-details/${id}`} style={{ color: '#333' }}>{title}</Link> {/* Darker text for title link */}
          </h5>
          <p style={{ color: '#666' }}>{description}</p> {/* Slightly lighter grey for description */}
          <ul className="blog-meta">
            <li style={{ color: '#777' }}>
              <i className="far fa-clock" style={{ color: '#007bff' }} /> {/* Blue clock icon */}
              {" "}
              {duration}
            </li>
            {guests && (
              <li style={{ color: '#777' }}>
                <i className="far fa-user" style={{ color: '#007bff' }} /> {/* Blue user icon */}
                {" "}
                {guests}
              </li>
            )}
          ...
          <div className="destination-footer">
            <span className="price" style={{ color: '#28a745', fontWeight: 'bold' }}> {/* Green and bold for price */}
            ...
            >
              ...
              <i className="fal fa-arrow-right" style={{ marginLeft: '5px' }} /> {/* Space for arrow */}
            </Link>
          </div>
        </div>
      </div>
